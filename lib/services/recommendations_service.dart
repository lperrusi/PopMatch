...
class RecommendationsService {
  ...
  static RecommendationsService get instance => _instance ??= RecommendationsService._();
  ...
  final Map<String, List<double>> _movieEmbeddings = {};
  ...
  Future<List<Movie>> getPersonalizedRecommendations(User user, {int limit = 20}) async {
    try {
      ...
      final allRecs = <Movie>[];
      allRecs.addAll(userBasedRecs.map((m) => _addAIWeight(m, _userPreferenceWeight, user)));
      allRecs.addAll(collaborativeRecs.map((m) => _addAIWeight(m, _collaborativeWeight, user)));
      allRecs.addAll(contentBasedRecs.map((m) => _addAIWeight(m, _contentBasedWeight, user)));
      allRecs.addAll(contextualRecs.map((m) => _addAIWeight(m, _contextualWeight, user)));
      allRecs.addAll(popularRecs.map((m) => _addAIWeight(m, _popularityWeight, user)));
  ...
  Future<void> _updateUserBehaviorPatterns(User user) async {
    try {
      ...
      for (final interaction in recentInteractions) {
        final movieId = interaction['movieId'];
        final type = interaction['type'];
        
        if (type == 'like') {
          _userBehaviorPatterns[movieId] = (_userBehaviorPatterns[movieId] ?? 0) + 1.0;
  ...
  Future<List<Movie>> _getAIUserBasedRecommendations(User user) async {
    ...
    for (final likedMovieId in user.likedMovies) {
      try {
        final movie = await _tmdbService.getMovieDetails(int.parse(likedMovieId));
        if (movie != null) {
          // Get similar movies using AI-enhanced similarity
          final similarMovies = await _tmdbService.getSimilarMovies(movie.id);
          final aiScoredMovies = await _applyAISimilarityScoring(similarMovies, movie);
    ...
    for (final genreId in userPreferences['preferredGenres'] ?? []) {
      try {
        ...
        final aiScoredGenreMovies = await _applyAIGenreScoring(genreMovies, genreId, user);
        ...
        if (movie != null) {
          ...
          if (movie.genreIds != null) {
            for (final genreId in movie.genreIds!) {
              ...
              _genreAffinities[genreId.toString()] = (_genreAffinities[genreId.toString()] ?? 0) + 1.0;
            }
          }
          ...
          try {
            ...
            for (final person in crew) {
              if (person['job'] == 'Director') {
                ...
                _directorAffinities[director] = (_directorAffinities[director] ?? 0) + 1.0;
              }
            }
          } catch (e) {
          ...
          if (movie.originalLanguage != null) {
            languageCounts[movie.originalLanguage!] = (languageCounts[movie.originalLanguage!] ?? 0) + 1;
          }
          
          if (movie.runtime != null) {
            runtimeCounts[movie.runtime!] = (runtimeCounts[movie.runtime!] ?? 0) + 1;
          }
          
          if (movie.voteAverage != null) {
            ratingCounts[movie.voteAverage!] = (ratingCounts[movie.voteAverage!] ?? 0) + 1;
          }
        }
      } catch (e) {
        // Continue with next movie
  ...
  Future<List<Movie>> _applyAISimilarityScoring(List<Movie> movies, Movie referenceMovie) async {
    ...
    for (final movie in movies) {
      ...
      if (movie.genreIds != null && referenceMovie.genreIds != null) {
        final commonGenres = movie.genreIds!.toSet().intersection(referenceMovie.genreIds!.toSet());
      ...
      if (movie.voteAverage != null && referenceMovie.voteAverage != null) {
        final ratingDiff = (movie.voteAverage! - referenceMovie.voteAverage!).abs();
  ...
  Future<List<Movie>> _applyAIGenreScoring(List<Movie> movies, int genreId, User user) async {
    final scoredMovies = <Movie>[];
    ...
    for (final movie in movies) {
      ...
      if (movie.genreIds != null) {
        for (final movieGenreId in movie.genreIds!) {
          final movieGenreAffinity = _genreAffinities[movieGenreId.toString()] ?? 0.0;
      ...
      for (final movie in popularMovies) {
        ...
        if (movie.year != null) {
          ...
          if (year != null) {
            ...
            final userPreferences = await _analyzeAIUserPreferences(user);
            final preferredYears = userPreferences['preferredYears'] as List<int>;
        ...
        if (movie.voteAverage != null) {
          final userPreferences = await _analyzeAIUserPreferences(user);
          final preferredRatings = userPreferences['preferredRatings'] as List<double>;
  ...
  Future<List<Movie>> _getAIContentBasedRecommendations(User user) async {
    ...
    try {
      ...
      for (final likedMovieId in user.likedMovies) {
        try {
          final likedMovie = await _tmdbService.getMovieDetails(int.parse(likedMovieId));
          if (likedMovie != null) {
            // Get similar movies with AI-enhanced content analysis
            final similarMovies = await _tmdbService.getSimilarMovies(likedMovie.id);
            ...
            for (final movie in similarMovies) {
              ...
              if (movie.genreIds != null && likedMovie.genreIds != null) {
                final commonGenres = movie.genreIds!.toSet().intersection(likedMovie.genreIds!.toSet());
              ...
              if (movie.voteAverage != null && likedMovie.voteAverage != null) {
                final ratingDiff = (movie.voteAverage! - likedMovie.voteAverage!).abs();
  ...
  List<Movie> _filterUserHistory(List<Movie> movies, User user) {
    final likedIds = user.likedMovies.map((id) => int.tryParse(id)).whereType<int>().toSet();
    final dislikedIds = user.dislikedMovies.map((id) => int.tryParse(id)).whereType<int>().toSet();
  ...
  Set<int> _getRecentlySkippedMovies() {
    ...
    // For now, we'll return an empty set and let the provider handle it
    return <int>{};
  }
  ...
  double _calculateMovieSimilarity(Movie movie1, Movie movie2) {
    ...
    if (movie1.genreIds != null && movie2.genreIds != null) {
      final commonGenres = movie1.genreIds!.toSet().intersection(movie2.genreIds!.toSet());
      final totalGenres = movie1.genreIds!.toSet().union(movie2.genreIds!.toSet());
