...
class BehaviorTrackingService {
  ...
  final Map<int, int> _movieRevisits = {}; // How many times user went back to a movie
  final Map<int, int> _detailViewCounts = {}; // How many times user viewed details
  final Map<String, List<int>> _swipeSequences = {}; // Track sequences of likes/dislikes
  
  // Track time spent on movie details
  final Map<int, List<Duration>> _detailViewDurations = {};
  
  // Track patterns
  final Map<int, double> _movieInterestScores = {}; // Calculated interest score per movie
  ...
  void recordMovieView(int movieId) {
    ...
    if (views.length > 1) {
      final timeDiff = views[views.length - 1].difference(views[views.length - 2]);
  ...
  void recordSwipe(String userId, int movieId, String action) {
    ...
    if (action == 'like' || action == 'match') {
      _movieInterestScores[movieId] = (_movieInterestScores[movieId] ?? 0.0) + 1.0;
    } else if (action == 'dislike') {
      _movieInterestScores[movieId] = (_movieInterestScores[movieId] ?? 0.0) - 0.5;
    }
  ...
  double getInterestScore(int movieId) {
    ...
    if (durations.isNotEmpty) {
      final avgDuration = durations.map((d) => d.inSeconds).reduce((a, b) => a + b) / durations.length;
      score += (avgDuration / 10.0).clamp(0.0, 2.0); // Max 2 points for time spent
    }
  ...
  Map<String, dynamic> getBehaviorInsights(String userId) {
    ...
    return {
      ...
      'mostViewedMovies': _detailViewCounts.entries
          .toList()
          ..sort((a, b) => b.value.compareTo(a.value))
          ..take(5)
          .map((e) => e.key)
          .toList(),
      'highInterestMovies': _movieInterestScores.entries
          .toList()
          ..sort((a, b) => b.value.compareTo(a.value))
          ..take(5)
          .map((e) => e.key)
