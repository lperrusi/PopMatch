...
class RecommendationMetricsService {
  static final RecommendationMetricsService _instance =
  ...
  Future<RecommendationMetrics> evaluateRecommendations({
    ...
    final dislikedMovieIds = user.dislikedMovies
    ...
    // Calculate metrics
    final precision =
        _calculatePrecision(topKIds, likedMovieIds, dislikedMovieIds);
  ...
  double _calculatePrecision(
      Set<int> recommendedIds, Set<int> likedIds, Set<int> dislikedIds) {
  ...
  double _calculateRecall(
      Set<int> recommendedIds, Set<int> likedIds, Set<int> shownIds) {
  ...
  double _calculateCoverage(
      List<Movie> recommendations, Set<int> shownMovieIds) {
  ...
  RecommendationMetrics getAverageMetrics({int? days}) {
    ...
    return RecommendationMetrics(
      precision:
          metricsToAverage.map((m) => m.precision).reduce((a, b) => a + b) /
              metricsToAverage.length,
      recall: metricsToAverage.map((m) => m.recall).reduce((a, b) => a + b) /
          metricsToAverage.length,
      ndcg: metricsToAverage.map((m) => m.ndcg).reduce((a, b) => a + b) /
          metricsToAverage.length,
      diversity:
          metricsToAverage.map((m) => m.diversity).reduce((a, b) => a + b) /
              metricsToAverage.length,
      novelty: metricsToAverage.map((m) => m.novelty).reduce((a, b) => a + b) /
          metricsToAverage.length,
      coverage:
          metricsToAverage.map((m) => m.coverage).reduce((a, b) => a + b) /
              metricsToAverage.length,
      timestamp: DateTime.now(),
  ...
  Future<void> _saveMetrics(RecommendationMetrics metrics) async {
    try {
      ...
      final existingMetrics =
          prefs.getStringList('recommendation_metrics') ?? [];
