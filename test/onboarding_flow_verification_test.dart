...
void main() {
  group('Onboarding Flow Verification', () {
    ...
    test(
        'COMPLETE FLOW: Sign up → Complete onboarding → Sign in again → Should skip onboarding',
        () async {
      ...
      print(
          '✅ User data saved. onboardingCompleted: ${userAfterSignup.preferences['onboardingCompleted']}');
      ...
      expect(onboardingCompleted, true,
          reason: 'onboardingCompleted should be true after onboarding');
      ...
      final userAfterOnboarding =
          User.fromJson(jsonDecode(userDataAfterOnboarding!));
      expect(userAfterOnboarding.preferences['onboardingCompleted'], true);
      print(
          '✅ User data persisted. onboardingCompleted: ${userAfterOnboarding.preferences['onboardingCompleted']}');
      ...
      final onboardingCompletedOnSecondLogin =
          newAuthProvider.userData?.preferences['onboardingCompleted'] ?? false;
      print(
          '🔍 onboardingCompleted on second login: $onboardingCompletedOnSecondLogin');
      print('📋 All preferences: ${newAuthProvider.userData?.preferences}');

      expect(onboardingCompletedOnSecondLogin, true,
          reason:
              'onboardingCompleted should be true on second login - user should skip onboarding');

      // Verify preferences are preserved
      expect(newAuthProvider.userData?.preferences['selectedGenres'], [28, 35]);
      expect(newAuthProvider.userData?.preferences['selectedPlatforms'],
          ['Netflix', 'Disney+']);
    ...
    test('VERIFY: User data loading by email in development mode', () async {
      // Use sign-up first so registered_users gets the correct password hash
      // (AuthService compares _hashPassword(password) to stored hash)
      const email = 'test2@example.com';
      const password = 'password123';

      final signUpUser = await authService.signUpWithEmailAndPassword(
        email,
        password,
        'Test User 2',
      );
      ...
      // Overwrite user_data with our user that has onboardingCompleted and preferences
      // so we can verify sign-in loads this data by email
      final user = User(
        id: signUpUser!.id,
        email: email,
      ...
      // Sign in with same credentials - should load existing user_data by email
      ...
      expect(onboardingCompleted, true,
          reason:
              'onboardingCompleted should be preserved when loading by email');

      // Verify all preferences are preserved
      expect(authProvider.userData?.preferences['selectedGenres'], [28]);
      expect(
          authProvider.userData?.preferences['selectedPlatforms'], ['Netflix']);
    });
  });
}
